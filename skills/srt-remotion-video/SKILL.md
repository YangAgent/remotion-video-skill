...
6. `beatPlan` 中每个 beat 都必须显式绑定 `segments`，不要写独立时间锚点字段
...
5. `beatPlan` 只负责声明哪些 `segments` 组成一个 beat；实际时间必须从 scenesData[].segments 的 `relativeStart / relativeDuration` 推导
6. 场景主节奏必须绑定 scenesData[].segments[].relativeStart / relativeDuration
7. 默认围绕画面中部或中上区域组织主视觉，先建立整屏构图区域，再在局部做 absolute 定位；不要让主视觉缩成局部小组件
8. 组件接口固定为 React.FC<{ segments: Segment[] }> 且使用默认导出
9. 只负责产出 {projectRoot}/src/scenes/SceneXXX.tsx；若文件不存在则创建，若已存在则仅修改自己负责的场景文件
10. 不要手改 {projectRoot}/src/compositions/Main.tsx 或 generated-scenes.ts
11. 完成后必须按参考协议中的“完成后返回”契约，返回结构化结果
12. `remotion-best-practices` 与当前 skill 同级，固定入口为 {skillRoot}/../remotion-best-practices/SKILL.md
```
