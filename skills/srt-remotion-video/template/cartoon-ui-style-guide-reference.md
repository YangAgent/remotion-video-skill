...
- 如果场景主要依赖图解、流程、关系或节点连接，单布局优先使用无框分层承托；`whiteboard-zone` 只作为局部元素容器或多分区子容器，不作为中央大白板
...
- 图解、流程、关系、节点连接、图表优先通过图形本身、空间分组、轻阴影、连接线、色块、标签和节奏动画建立层级
...
- 当画面本身已有足够节点、箭头、标签和图形关系时，可以直接使用透明根层 + 局部承托，不必额外补一个大底板
